    ...
    public function get($type, $value){
        ...
        switch ($type) {
			...
			case 'select':
				...
				$sql = "SELECT `existencias` AS `max`, `precio_venta` AS `precio`, `nombre_producto` AS `nombre`, `codigo_producto` AS `codigo`, `marca_producto` AS `marca`, `tipo_producto` AS `tipo` FROM `productos` ORDER BY `nombre` ASC";
				$sql_values = null;
				$consulta = $this->conexion->get_consulta($sql, $sql_values);
